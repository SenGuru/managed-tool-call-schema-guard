...
| Command                                            | Result                                                                                                                                                                                                                                                                |
| -------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run check`                                    | latest clean rerun passed in 21.43 s with the retained browser temporarily paused: format, lint, scripts, package boundary, types, dry provider boundary and conformance; 271 JS/TS passed, 19 PostgreSQL skipped; 5 Python passed; zero live provider trials claimed |
...
| isolated `Dockerfile.load-audit` run               | exact revision `65c7bd5` passed 2,000/2,000 requests in a hardened two-CPU/2 GiB container on idle DreamHost; 361.87 req/s, p95 99.55 ms, exact metering, valid audit/release chains, no persisted private sentinel; live staging remained healthy                    |
...
- isolated DreamHost managed HTTP load, 2,000 requests at concurrency 32:
  361.87 requests/s, p50 81.73 ms, p95 99.55 ms, p99 437.98 ms, zero errors,
  exact metering, 2,000 unique audit IDs, valid audit/release chains and no
  private sentinel;
