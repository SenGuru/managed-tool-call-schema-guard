# Production blueprint audit — 2026-07-22 (updated 2026-07-23)
...
| Declared repository gate                   | `npm run check`                                                                                                                                                             | Passed: formatting, build, lint, syntax, package contents, typecheck, conformance, 183 TS tests and 4 Python tests; 15 PostgreSQL tests skipped without a credentialed database                                  | Local, SQLite and in-memory/shared contracts                              |
| PostgreSQL suite and coverage              | credentialed `npm run test:coverage` against PostgreSQL 16                                                                                                                  | 198/198 tests in 16.31 s; lines 81.59%, statements 79.63%, branches 73.72%, functions 80.98%                                                                                                                     | Ephemeral loopback PostgreSQL, not deployed failover                      |
| Severe local gate                          | `npm run audit:extreme`                                                                                                                                                     | Passed in 52.46 s after dependency and evidence-redaction fixes                                                                                                                                                  | Local process/SQLite boundary                                             |
...
| Production-image lifecycle                 | `npm run audit:container-e2e`                                                                                                                                               | Passed in 18.647 s on 2026-07-23; actual managed, PostgreSQL, TLS proxy and anchor containers; local/shared lifecycle synchronization, export/deletion lock, receiver outage and three restart classes exercised | One Docker host and one bridge network, not separate machines             |
...
| Public managed workflow                    | `npm run audit:public-managed` with an owner-only key file and exact disposable audit tenant                                                                                | 61 requests in 18.077 s on exact r3: registry/release/admission, all decision outcomes, key lifecycle, policy, ruleset, action/anchor, integrity, export and lock                                                | Real TLS/proxy/PostgreSQL/anchor boundary; no downstream tool execution   |
| Exact-r3 separate-host outage              | `npm run audit:public-anchor-outage` with explicit deployed revision, owner-only key and exact anchor edge container                                                        | Passed in 18.771 s: public admission returned `checkpoint_anchor_unacknowledged`; independent edge recovered in 7.622 s; reservation remained duplicate-blocked and integrity verified                           | Real DreamHost-to-DigitalOcean failure boundary                           |
| Exact-r3 in-app-browser workflow           | Browser dashboard at `https://api.akriven.com/dashboard` with a disposable audit tenant                                                                                     | Active lifecycle and every dashboard panel loaded; export downloaded with hash; wrong deletion rejected; correct deletion locked access; locked reload suppressed operational panels; export remained available  | Real browser/TLS/dashboard/API boundary; not every API has a dashboard UI |
...
| Core benchmark                             | included by `audit:extreme`                                                                                                                                                 | 10,000 iterations under concurrent desktop load: p50 57.83 µs, p95 88.25 µs, p99 164.88 µs                                                                                                                       | Local CPU microbenchmark; not a production SLO                            |
| Managed load                               | included by `audit:extreme`                                                                                                                                                 | 2,000/2,000 HTTP 200, 347.27 req/s, p50 73.74 ms, p95 100.71 ms, p99 930.95 ms, max 1,494.70 ms; exact metering and audit chain                                                                                  | Single local managed process and SQLite; not a production SLO             |
...
the credentialed PostgreSQL suite, production-container inspection, a 61-request
public run, and the in-app browser flow all prove both projections are
`deletion_pending` before exact-export-hash deletion. The deployed r3 image is
...
ciphertext was verified on the DigitalOcean failure domain. The scanned
`linux/amd64` r3 managed image (UID/GID 65532) reported zero HIGH/CRITICAL
vulnerabilities and zero embedded secrets. Its public 61-request workflow,
separate-host outage drill, in-app-browser deletion workflow, and offline
local/shared inspection all passed. Seven signed deletion receipts now remain
in each active store, no `audit-*` tenant remains, SQLite integrity is `ok`, and
the exact r3 image is still running.
...
| Managed API daily workflow                                                                              | `packages/managed/src/server.ts`                                           | extensive HTTP tests                                             | Broad image E2E route lifecycle                                                                                                                     | proven on protected API-key model                | Suitable for operator-led private cohort                                            |
...
| Tenant bootstrap and API-key scopes/rotation/revocation                                                 | offline-guarded bootstrap, managed/shared control                          | auth/isolation/revocation/tamper/concurrency tests               | Public stop/bootstrap/restart measured 3 s; issue/revoke in public E2E                                                                              | proven for operator-led API keys                 | Online self-serve provisioning, human identity and recovery remain absent           |
...
| Tenant suspension/cancellation and complete deletion lifecycle                                          | HMAC-bound local/shared lifecycle, API gate, operator CLI and receipts     | migration, tamper, tenant, shared PG, SDK/CLI tests              | Production-container and in-app-browser request/export/locked-state workflow; disposable offline deletion                                           | proven for operator-led workflow                 | Legal retention policy and real paid-tenant drill remain external                   |
...
| Checkout, signed billing webhooks, failed payment, cancellation and entitlement reconciliation          | none; statement returns `integration_required`                             | explicit negative boundary test only                             | none                                                                                                                                                | missing                                          | Must-fix before automated charging; manual invoicing only for private cohort        |
| Durable alert webhooks, retry/dead-letter/redrive                                                       | managed/shared alerts/webhook transport                                    | SSRF/signature/retry/shared tests                                | Loopback host receiver failure/recovery and scheduled monitors proved; no owned external receiver                                                   | partially proven                                 | Real receiver and external dead-letter monitor required                             |
...
2. Sandbox-tested checkout/customer portal and raw-body signed billing webhooks
   with replay, duplicate, reordering, failure and reconciliation tests.
...
are proven. Automated billing and self-service must remain disabled.
