...
| ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run check`                                                                      | Latest clean full rerun passed in 25.09 seconds with the retained browser temporarily paused: format, build, ESLint, script syntax, dry provider boundary, package boundary, typecheck, conformance, 276 JavaScript/TypeScript tests passed, 19 credentialed PostgreSQL tests skipped, and 5 Python tests passed. The dry provider boundary reported zero live trials, as required. An earlier bounded full run passed in 236.81 seconds under heavier host contention. The unrelated untracked `apps/` tree was preserved.                                                                                                                                                                                |
...
| `cd website && npm ci && npm run lint && npm test && npm audit --audit-level=high`   | The exact-lockfile install completed from a clean dependency tree. ESLint 10.8.0, the direct Next.js 16.2.12 rules, React Hooks rules and TypeScript rules passed; the five-stage production build exposed 18 route definitions; all 3 rendered HTML/trust suites passed; and npm reported 0 known vulnerabilities across 405 installed packages. The upgrade removed the vulnerable `react-server-dom-webpack` 19.2.6 runtime and the incompatible legacy `brace-expansion` lint path rather than overriding it.                                                                                                                                                                                          |
| In-app browser website and activation traversal                                      | The owner-authenticated browser traversed all 17 static paths and all four resource-article paths in private Sites version 16. Every one of the 21 documents had the expected URL, a unique title, `main`, `h1`, non-empty body and no application-error page. It exercised all three Product/Developers/Resources menus, all three TypeScript/CLI/Managed API quickstart tabs, both activation branches and copy feedback. The managed handoff opened `https://api.akriven.com/dashboard/overview`, rendered the unauthenticated dashboard shell and sign-in/API-key connection boundary, and did not expose raw `authentication_required` JSON. Authenticated re-login remains pending a fresh MFA code. |
...
- verified the internal-evaluation quota and retention, exact 90-day
  ...
  Portal controls;
- rebuilt exact website commit
  `67580263d6e2e59144564532008003251d239b5b` after a clean `npm ci`, then
  traversed all 17 static routes plus all four resource-article routes in the
  owner-only Sites version 16; every route exposed its expected URL, a unique
  title, `main`, `h1`, non-empty body and no application-error page;
- exercised the Product, Developers and Resources menus, all three quickstart
  tabs, both activation branches, copy feedback and the real managed-workspace
  handoff;
- checked production worker error logs after traversal. No exception or
  server-error event occurred. Two informational canceled RSC prefetches were
  produced by rapid navigation and did not correspond to a failed document
  response.
...
passed with 276 JS/TS tests, 19 credentialed-PostgreSQL tests skipped by the
